...
현재 공개 버전: **0.5.5**. 기본 분석은 규칙 기반이며 모델 가중치나 AI 서버가 필요하지 않습니다.

## 실행

[Releases](https://github.com/sukwoo0711-maker/FlowNote/releases/tag/v0.5.5)에서
`FlowNote-0.5.5-win-x64-core-setup.exe` 설치본 또는 `FlowNote-0.5.5-win-x64-core.zip` 포터블을 받습니다.
...
- 플로팅 기록 창이 노란 포스트잇으로 바뀌었습니다. 한 줄 크기 520×52는 유지합니다.
- 최근 기록은 펼치지 않아도 본문이 보이는 작은 종이 3장입니다.
- 하루 보기와 파노라마의 메모도 같은 종이입니다. 파노라마에서 원문을 따로 열 필요가 없습니다.
- 빈 하루는 빈 종이를 누르면 바로 적습니다.
- v0.5.4의 퀵 단축키·단일 본문·사진 확대를 유지합니다. 새 모델·타이머는 추가하지 않았습니다.
...
검수와 제한 사항은 `docs/releases/0.5.5.md`를 참고하세요.

## 퀵 메모 · 사진 보기

기본 단축키는 Ctrl+Alt+Space입니다. 이미 다른 앱이 사용 중이면 Ctrl+Alt+N으로 등록하며 실제 키는 설정과 트레이에 표시합니다.
Shift+Enter로 제목 없이 여러 줄을 쓰고 Ctrl+Enter로 기록합니다. 하루 보기의 + 메모도 같은 입력으로 연결됩니다.
사진을 클릭하면 앱 안에서 확대합니다. 확대/축소·화면에 맞춤·Ctrl+휠을 지원하며 Escape로 닫습니다.
